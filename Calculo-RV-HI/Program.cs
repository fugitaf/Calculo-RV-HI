using Calculo_RV_HI.Entities;
using Calculo_RV_HI.Utils;
...
namespace Calculo_RV_CM
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string pathSld = @"C:\Users\fefug_skli85i\Documents\Temp_RV_HI\SLD.csv";
            string pathApl = @"C:\Users\fefug_skli85i\Documents\Temp_RV_HI\APL.csv";
            Console.WriteLine("------------------------------------------------------------------------");
            ...
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (fields[0] != "DTULTRIB")
                    {
                        ...
                        decimal saldoAmortizacaoDePrincipal = decimal.Parse(fields[4], new CultureInfo("pt-BR"));
                        ...
                        certificado.CotacaoUltimaTributacao = decimal.Parse(fields[3], new CultureInfo("pt-BR"));
                        certificado.SaldoAmortizacaoDePrincipal = decimal.Parse(fields[4], new CultureInfo("pt-BR"));
                        certificado.valorUfirAplicacao = decimal.Parse(fields[5], new CultureInfo("pt-BR"));
            ...
            listaPosicaoAnalitica = Utils.CalculaPosicaoAnalitica(listaCertificados, saldo, fundos);

            // Grava Informações

            Utils.GravaDadosDeEntrada(saldo, fundos, bloqueios);

            Utils.GravaPeriodosDoCertificado(listaPosicaoAnalitica);

            Utils.GravaCertificadosCalculados(listaPosicaoAnalitica);

            Utils.GravaSaldoConsolidado(listaPosicaoAnalitica, bloqueios);
